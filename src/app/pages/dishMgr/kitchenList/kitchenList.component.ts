...
import { DishMgrService, KitchenListParams, EditKitchenParams } from '../dishMgr.service';
import swal from 'sweetalert';
@Component({
  selector: 'app-kitchenList',
  templateUrl: './kitchenList.component.html',
  styleUrls: ['./kitchenList.component.css']
})
export class KitchenListComponent implements OnInit {

  params: KitchenListParams = new KitchenListParams();
      ...
      search(reset: boolean = false) {
          ...
          this._loading = true;
          this.service.getKitchenList(this.params).subscribe(res => {
              this._loading = false;
              if (res.State == 0) {
      ...
      edit(item: any = {}) {
          swal({
              ...
              .then(text => {
                  if (text) {
                      let params: EditKitchenParams = new EditKitchenParams();
                      params.Id = item.Id || 0;
                      params.SName = text;
                      this.service.editKitchen(params).subscribe((res) => {
                          if (res.State == 0) {
                              if (item.SName) {
                                  item.SName = text;
                              } else {
